<template lang="pug">
  b-container#settings-main(v-if="isSettings")
...
export default {
  ...
  computed: {
    ...
    isChanged() {
      ...
      for (const property in this.settings) {
        if (this.settings[property] != this.settingsWatcher[property])
          return true;
      }
  ...
  methods: {
    ...
    async saveSettings() {
      const res = await fetchutil.postData(
        `${this.apihost}/user/settings/save/`,
        this.settings
      );
      const body = await res.json();
      if (res.status == 200) {
